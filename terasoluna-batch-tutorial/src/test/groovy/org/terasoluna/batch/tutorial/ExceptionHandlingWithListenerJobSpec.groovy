...
@Slf4j
class ExceptionHandlingWithListenerJobSpec extends Specification {
    ...
    def "Confirm that Exception Handling With Listener by ChunkModel Job has occurred Exception"() {
        when:
        def exitCode = launcher.syncJob {
            JobLauncher.SyncJobArg arg ->
                arg.jobRequest = new JobRequest(
                        jobName: 'jobPointAddChunk',
                        jobFilePath: 'META-INF/jobs/exceptionhandlingwithlistener/jobPointAddChunk.xml')
    ...
    def "Confirm that Exception Handling With Listener by TaskletModel Job has occurred Exception"() {
        when:
        def exitCode = launcher.syncJob {
            JobLauncher.SyncJobArg arg ->
                arg.jobRequest = new JobRequest(
                        jobName: 'jobPointAddTasklet',
                        jobFilePath: 'META-INF/jobs/exceptionhandlingwithlistener/jobPointAddTasklet.xml')
                arg.sysprop = ['tutorial.insert-data.script=file:sqls/insert-member-info-error-data.sql']
        }
